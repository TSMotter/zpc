...
yahdlc_control_t yahdlc_get_control_type(unsigned char control)
{
    ...
    if (control & (1 << YAHDLC_CONTROL_S_OR_U_FRAME_BIT))
    {
        ...
        {
            // Assume it is an NACK since Receive Not Ready, Selective Reject and U-frames are not
            // supported
    ...
    {
        // It must be an I-frame so add the send sequence number (receive sequence number is not
        // used)
...
int yahdlc_get_data(yahdlc_control_t *control, const char *src, unsigned int src_len, char *dest,
                               ...
                               unsigned int src_len, char *dest, unsigned int *dest_len)
{
    ...
    else
    {
        ...
        {
            // Return FCS error and indicate that data up to end flag sequence in buffer should be
            // discarded
            *dest_len = i;
            ret       = -EIO;
        }
...
int yahdlc_frame_data(yahdlc_control_t *control, const char *src, unsigned int src_len, char *dest,
                      unsigned int *dest_len)
{
    unsigned int  i;
    int           dest_index = 0;
